...
def convert_py_to_md(py_file: Path, md_file: Path) -> None:
    ...
    with open(py_file, 'r') as f:
        ...
        if line1.lower().startswith('leetcode'):
            title = re.match(r'[\w\s]*\d+\.(.+)', line1).groups()[0].strip()

            # remove non alphamuneric chars, and reduce consecutive '-'
            clean_title = re.sub(r'[^\w\s-]', '', title.lower())
            title_slug = re.sub(r'[-\s]+', '-', clean_title)
