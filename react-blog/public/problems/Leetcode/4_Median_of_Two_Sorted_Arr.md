...
This method guarantees both target removal AND tight prefix removal. Time O(log(m+n))


Method 3, binary search nums1, in loop body bisect on nums2 with nums1[mid], O(log(m)*log(n))
This method guarantees 2nd condition of no "holes", binary search by prefix number count.
...
e.g. nums1 = [1,2,5,7,8], nums2= [2,4,6]. Total = 8, m1+m2+2 = 8//2+1 = 5
1. m1 = 2(5), m2 thus 1(4). This is valid because all nums2[,m2] < nums1[m1], and all nums2[m2+1,] >= nums1[m1]
	=> nums1[m1] separates nums2 in the same way as nums2[m2] separate nums1
    => no "holes" after merging into arr[0,median]
...
Then finally check & output the correct median in all cases.
...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        # because here nums2 search by count, not by controlling its idx
        if n1 > n2:
